 ...
 setlocale(LC_ALL, 'C.UTF-8', 'C');
    ...
    if ($cat_layout_text != ''){
        ...
        if($total_subcategories > 0){
            ...
            for ($i=0; $i < $total_subcategories; $i++){
                ...
                if ($show_empty_categories || $jlistConfig['view.empty.categories'] || $this->children[$parentCatid][$i]->getNumItems(true) || count($this->children[$parentCatid][$i]->getChildren())){            
                    ...
                    //display cat info
                     ...
                     $body_subcats = str_replace('{cat_pic}', '', $body_subcats);
                     $body_subcats = str_replace('{cat_info_begin}', '', $body_subcats); 
                     $body_subcats = str_replace('{cat_info_end}', '', $body_subcats);                     
                     $body_subcats = str_replace('{cat_title}', '', $body_subcats); 
                     $body_subcats = str_replace('{sum_subcats}', '', $body_subcats); 
                     $body_subcats = str_replace('{sum_files_cat}', '', $body_subcats); 
            ...
            if ($jlistConfig['use.pagination.subcategories'] && $total_subcategories){
                // add script 
    ...
    if ($layout_files_text != ''){
        ...
        $body_cat .= JHtml::_( 'form.token' ).'</form>';
...
$db->setQuery('select * from #__jdownloads_categories where id='.$db->quote(JRequest::getVar('catid')));
$res = $db->loadObject();
if ($res) {
	if (substr($res->alias,0,2)=='sz') {
	   // lehet hogy szavazáshoz rendelt kategoria
	   $szavazas_id = substr($res->alias,2,10);
	   $db->setQuery('select * from #__jdownloads_categories where id = '.$res->parent_id);
	   $res = $db->loadObject();
	   if ($res) {
		   if (substr($res->alias,0,1) == 't') {
