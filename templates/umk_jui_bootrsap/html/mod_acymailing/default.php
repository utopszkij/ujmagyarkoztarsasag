...
<?php
	...
	<div class="acymailing_fulldiv" id="acymailing_fulldiv_<?php echo $formName; ?>" <?php echo $styleString; ?> >
		...
		<div class="acymailing_module_form" >
			...
			if($params->get('dropdown',0)){
				...
				$listContent .= '<var class="acymailing_lists" id="acymailingPopup" style="display:none; position:absolute; z-index:99;">
				<table>
				<tr class="popupHead"><td align=right><button type="button" 
				    class="btnPopupClose" onclick="acymailingCloseClick()" title="Bezár">&nbsp;</button></td></tr>
				';
			...
			<table class="acymailing_form">
				...
				if(empty($identifiedUser->userid) AND $config->get('captcha_enabled') AND acymailing_level(1)){ ?>
				 ...
				 if($params->get('showterms',false)){
			       ...
			       <td><button type="button" class="btn btnPopup" onclick="acymailingPopupClick()">Válasszon témaköröket!</button> </td>
	...
	<script type="text/javascript">
	  function acymailingPopupClick() {
		  var itemCount = <?php echo count($visibleListsArray); ?>;
		  jQuery('.acymailing_lists').css('margin-top',(-33*(itemCount+1))+'px');
		  //jQuery('.acymailing_lists').css('display','block');
		  jQuery('.acymailing_lists').show(500);
	  }
	  function acymailingCloseClick() {
