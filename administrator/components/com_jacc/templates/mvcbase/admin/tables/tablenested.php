<?php
/**
 * @version		$Id: tablenested.php 125 2012-10-09 11:09:48Z michel $
 ...
 */
class ##Component##JTableNested extends JTable
{
	/**
	...
	public function move($referenceId, $position = 'after', $pk = null)
	{
		...
		if ($node->parent_id != $repositionData->new_parent_id) {
			$this->_db->setQuery(
				...
				' WHERE `'.$this->getKeyName().'` = '.(int) $node->$k
			);
			$this->_db->query();
	...
	public function orderUp($pk)
	{
		...
		$this->_db->setQuery(
			...
			' AND `'.$this->getKeyName().'` NOT IN ('.implode(',', $children).')'
		);
		$this->_db->query();
	...
	public function orderDown($pk)
	{
		...
		$exclude = ($children) ? ' AND `'.$this->getKeyName().'` NOT IN ('.implode(',', $children).')' : '';
		// Shift left and right values for the sibling and it's children.
		$this->_db->setQuery(
			'UPDATE `'.$this->_tbl.'`' .
			' SET `lft` = `lft` - '.(int) $node->width.', `rgt` = `rgt` - '.(int) $node->width .
