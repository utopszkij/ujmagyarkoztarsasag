...
<form action="index.php?option=com_jacc&amp;view=plugins" method="post" id="adminForm" name="adminForm">
	<table>
		<tr>
...
<?php
  ...
  for ($i=0, $n=count( $this->items ); $i < $n; $i++):
    ...
    if (JRequest::getVar('function', null)) {
    	$onclick= "onclick=\"window.parent.jSelectPlugins_id('".$row->id."', '".$this->escape($row->name)."', '','id')\" ";
    }  	
    
 	$link = JRoute::_( 'index.php?option=com_jacc&view=plugins&task=edit&cid[]='. $row->id );
 	$row->id = $row->id; 	
 	$checked = JHTML::_('grid.id', $i, $row->id); 	
  	$published = JHTML::_('grid.published', $row, $i ); 	
 	$archive = 'plg_'.JFilterOutput::stringURLSafe($row->name).'-'.$row->version.'.zip';
  ?>
