...
<form action="index.php?option=com_jacc&amp;view=packages" method="post" name="adminForm" id="adminForm">
	<table>
		<tr>
	...
	<table class="adminlist table table-striped">
		<thead>
			<tr>
				...
				<th  width="150"class="title"><?php echo JText::_('Download') ?></th>								
				<th width="20" class="title">
...
<?php
  ...
  for ($i=0, $n=count( $this->items ); $i < $n; $i++):
    ...
    if (JRequest::getVar('function', null)) {
    	$onclick= "onclick=\"window.parent.jSelectPackages_id('".$row->id."', '".$this->escape($row->name)."', '','id')\" ";
    }  	
    
 	$link = JRoute::_( 'index.php?option=com_jacc&view=packages&task=edit&cid[]='. $row->id );
 	$row->id = $row->id; 	
  ...
  ?>
	<tr class="<?php echo "row$k"; ?>">
		...
		<td>				
			<a href="<?php echo JURI::base() ?>components/com_jacc/archives/pkg_<?php echo $row->alias ?>-<?php echo $row->version ?>.zip">pkg_<?php echo $row->alias ?>-<?php echo $row->version ?>.zip</a>
		</td>        
