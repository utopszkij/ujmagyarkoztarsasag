...
defined('_JEXEC') or die('Restricted access');
  ...
  JToolBarHelper::title( JText::_( 'Szavazasok' ), 'generic.png' );
  JToolBarHelper::addNew();
  JToolBarHelper::editList();
  JToolBarHelper::publishList();
  JToolBarHelper::unpublishList();  
  JToolBarHelper::deleteList();  
  JToolBarHelper::preferences('com_szavazasok', '550');  
?>

<form action="index.php?option=com_szavazasok&amp;view=szavazasok" method="post" name="adminForm" id="adminForm">
	<table>
		<tr>
...
<?php
  ...
  for ($i=0, $n=count( $this->items ); $i < $n; $i++):
    ...
    if (JRequest::getVar('function', null)) {
    	$onclick= "onclick=\"window.parent.jSelectSzavazasok_id('".$row->id."', '".$this->escape($row->megnevezes)."', '','id')\" ";
    }  	
    
 	$link = JRoute::_( 'index.php?option=com_szavazasok&view=szavazasok&task=edit&cid[]='. $row->id );
 	$row->id = $row->id; 	
...
<input type="hidden" name="task" value="szavazasok" />
