...
class BeallitasokHelper
{
	
	/*
	 * Submenu for Joomla 1.6
	 */
	public static function addSubmenu($vName = 'beallitasok')
	{
        
		JSubMenuHelper::addEntry(
			JText::_('Beallitasok'),
			'index.php?option=com_beallitasok&view=beallitasok',
			($vName == 'beallitasok')
		);
	...
	public static function getExtensions() 
	{
		...
		$xml = simplexml_load_file(JPATH_ADMINISTRATOR.'/components/com_beallitasok/elements/extensions.xml', 'JXMLElement');		        
 ...
 * @since		1.5
 */
abstract class JHtmlBeallitasok
{
	/**
	...
	{
		$extensions = BeallitasokHelper::getExtensions();
		$options = array();
		...
		foreach ($extensions as $extension) {   
			...
			$option->value = 'com_beallitasok.'.$extension->name;
			$options[] = $option;			
		}		
	...
	public static function categories($extension, $cat_id,$name="categories",$title="Select Category", $config = array('attributes'=>'class="inputbox"','filter.published' => array(0,1)))
	{
			...
			$query->from('#__beallitasok_categories AS a');
