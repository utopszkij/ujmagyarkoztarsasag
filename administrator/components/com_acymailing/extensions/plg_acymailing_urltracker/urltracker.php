...
class plgAcymailingUrltracker extends JPlugin
{
	...
	function acymailing_replaceusertags(&$email,&$user,$send=true){
		...
		foreach($results[1] as $i => $url){
			...
			if(!$isFile && rtrim($url, '/') != rtrim(ACYMAILING_LIVE, '/') && strpos($url,ACYMAILING_LIVE) !== false){
				$link = str_replace('/', DS, str_replace(ACYMAILING_LIVE, ACYMAILING_ROOT, $url));
				if(strpos($link, 'index.php') === false && strpos($link, 'index2.php') === false && @file_exists($link)) $isFile = true;
			}
	...
	function onAcyDisplayTriggers(&$triggers){
	 ...
	 function onAcyDisplayFilters(&$type,$context="massactions"){
		...
		if($this->params->get('displayfilter_'.$context,true) == false) return;
	 	...
	 	$db = JFactory::getDBO();
		$db->setQuery("SELECT `mailid`,CONCAT(`subject`,' ( ',`mailid`,' )') as 'value' FROM `#__acymailing_mail` WHERE `type` IN('news','autonews','followup') ORDER BY `senddate` DESC LIMIT 5000");
