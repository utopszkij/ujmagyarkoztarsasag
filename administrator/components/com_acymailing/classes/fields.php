...
<?php
	...
	function display($field,$value,$map,$inside = false){
		...
		if(!$app->isAdmin() && !empty($this->origin) && in_array($this->origin, array('module','subform','joomla'))){
			if((isset($field->options['editablecreate']) && $field->options['editablecreate'] == 0 && empty($this->currentUser)) ||
				(isset($field->options['editablemodify']) && $field->options['editablemodify'] == 0 && !empty($this->currentUser))){
	...
	private function _displayDate($field,$value,$map,$inside){
		...
		if(ACYMAILING_J30 && strlen($value) > 0) $value = strftime('%Y-%m-%d', strtotime($value));
	...
	public function prepareConditionalDisplay($fieldsDisplayed, $prefix, $page, $formid = 'adminForm'){
		...
		foreach($fieldsDisplayed as $fieldName => $oneField){
			...
			do {
				...
				if(is_numeric($value)) $finCond = $operator ." ".$value;
				...
				if(in_array($condFieldName, array('email', 'name')) && in_array($page, array('userProfile','modifyForm'))){
					...
					if(ACYMAILING_J25) $currentFieldCond .= "(document.getElementsByName('jform['+'".$condFieldName."]')[0] && document.getElementsByName('jform['+'".$condFieldName."]')[0].value ".$finCond.")";
					...
					if($value != '') $currentFieldCond .= "(getMyElement(varform, prefix+'[".$condFieldName."][num]', 'name') && (getMyElement(varform, prefix+'[".$condFieldName."][country]', 'name').value +','+ getMyElement(varform, prefix+'[".$condFieldName."][num]', 'name').value ".$finCond."))";
			...
			if(!empty($currentFieldCond)){
				if(in_array($page, array('userProfile', 'modifyForm'))) $elem = "document.getElementById('tr".$oneField->namekey."')";
		...
		if(in_array($page, array('userProfile', 'modifyForm'))) $fctName = 'updateDisplayLimited_'.$page;
