...
<?php
	...
	while(isset($this->lines[$i])){
		if(empty($this->lines[$i])){ unset($this->lines[$i]); continue; }
		else $nbPreviewLines++;
		...
		if(strpos($this->lines[$i], '@')) $noHeader = 1;
		else $noHeader = 0;
	...
	<table class="adminlist" cellspacing="10" cellpadding="10" align="center" id="importdata">
		<?php
			...
			foreach($columnNames as $key => &$oneColumn){
				...
				if(empty($default) && $default !== 0){
					...
					if(!$default && !empty($firstValueLine)){
						if(strpos($firstValueLine[$key], '@')) $default = 'email';
			...
			for($i = 1-$noHeader ; $i < 11-$noHeader && $i < $nbLines ; $i++){
				...
				echo '<tr class="row'.(1-$i%2).'"><td align="center"><strong>'.($i+$noHeader).'</strong></td><td>'.implode('</td><td>', $values).'</td></tr>';
			}
		?>
	</table>
