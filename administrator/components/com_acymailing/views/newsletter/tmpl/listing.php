			...
			?>
				<tr class="<?php echo "row$k"; ?>">
					...
					<td align="center" style="text-align:center" >
						...
						<br/><button class="btn btn-primary" onclick="if(confirm('<?php echo str_replace("'", "\'", JText::sprintf('ACY_VALID_DELETE_FROM_QUEUE', $row->countqueued)); ?>')){ window.location.href = '<?php echo JURI::base(); ?>index.php?option=com_acymailing&ctrl=<?php if(!JFactory::getApplication()->isAdmin()) echo 'front'; ?>newsletter&task=cancelNewsletter&<?php echo acymailing_getFormToken(); ?>=1&mailid=<?php echo $row->mailid; ?>'; } return false;"><?php echo JText::_('ACY_CANCEL'); ?></button>
						<?php } ?>
					</td>
					<td align="center" style="text-align:center" >
						<?php
