				...
				<table class="adminlist">
					...
					foreach($this->mailsdetails as $oneMail){
						...
						if(!empty($this->savedValues)){
							...
							if($sent != 0){
								if(acymailing_level(3)) $cleanSent = $sent - $bounceunique;
								else $cleanSent = $sent;
							...
							echo '<td style="text-align:center">'.$open.' / '.$cleanSent.' ('.$openPrct.'%)</td>';
							echo '<td style="text-align:center">'.$click.' / '.$cleanSent.' ('.$clickPrct.'%)</td>';
