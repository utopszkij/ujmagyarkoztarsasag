...
<form action="index.php?option=<?php echo ACYMAILING_COMPONENT ?>&amp;ctrl=template" method="post" name="adminForm"  id="adminForm" >
	...
	<table class="adminlist table table-striped table-hover" cellpadding="1" id="templateListing">
		<thead>
			<tr>
				...
				<tr class="<?php echo "row$k"; ?>">
					...
					<td>
						<?php if(!empty($row->thumb)){ ?>
							...
							<img src="<?php echo rtrim(JURI::root(),'/').'/'.strip_tags($row->thumb) ?>" style="float:left;width:100px;margin-right:10px;" />
							</a>
						<?php } ?>
