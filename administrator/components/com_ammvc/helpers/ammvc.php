...
class AmmvcHelper
{
	 ...
	 */
	public static function addSubmenu($vName = 'amcomponents')
	{
        
		JSubMenuHelper::addEntry(
			JText::_('Amcomponents'),
			'index.php?option=com_ammvc&view=amcomponents',
			($vName == 'amcomponents')
		);

		JSubMenuHelper::addEntry(
			JText::_('Categories'),
			'index.php?option=com_ammvc&view=categories',
			($vName == 'categories')
		);
	...
	public static function getExtensions() 
	{
		...
		$xml = simplexml_load_file(JPATH_ADMINISTRATOR.'/components/com_ammvc/elements/extensions.xml', 'JXMLElement');		        
...
abstract class JHtmlAmmvc
{
	...
	public static function categories($extension, $cat_id,$name="categories",$title="Select Category", $config = array('attributes'=>'class="inputbox"','filter.published' => array(0,1)))
	{
			...
			$query->from('#__ammvc_categories AS a');
