...
defined('_JEXEC') or die('Restricted access');
  ...
  JToolBarHelper::title( JText::_( 'Tagok' ), 'generic.png' );
  JToolBarHelper::addNew();
  JToolBarHelper::editList();
  JToolBarHelper::publishList();
  JToolBarHelper::unpublishList();  
  JToolBarHelper::deleteList();  
  JToolBarHelper::preferences('com_tagok', '550');  
?>

<form action="index.php?option=com_tagok&amp;view=tagok" method="post" name="adminForm" id="adminForm">
	<table>
		<tr>
...
<?php
  ...
  for ($i=0, $n=count( $this->items ); $i < $n; $i++):
    ...
    if (JRequest::getVar('function', null)) {
    	$onclick= "onclick=\"window.parent.jSelectTagok_id('".$row->id."', '".$this->escape($row->id)."', '','id')\" ";
    }  	
    
 	$link = JRoute::_( 'index.php?option=com_tagok&view=tagok&task=edit&cid[]='. $row->id );
 	$row->id = $row->id; 	
...
<input type="hidden" name="option" value="com_tagok" />
<input type="hidden" name="task" value="tagok" />
<input type="hidden" name="view" value="tagok" />
