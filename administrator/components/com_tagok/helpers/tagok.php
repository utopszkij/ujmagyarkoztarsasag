...
class TagokHelper
{
	 ...
	 */
	public static function addSubmenu($vName = 'tagok')
	{
        
		JSubMenuHelper::addEntry(
			JText::_('Tagok'),
			'index.php?option=com_tagok&view=tagok',
			($vName == 'tagok')
		);
	...
	public static function getExtensions() 
	{
		...
		$xml = simplexml_load_file(JPATH_ADMINISTRATOR.'/components/com_tagok/elements/extensions.xml', 'JXMLElement');		        
...
abstract class JHtmlTagok
{
	...
	{
		$extensions = TagokHelper::getExtensions();
		$options = array();
		...
		foreach ($extensions as $extension) {   
			...
			$option->value = 'com_tagok.'.$extension->name;
			$options[] = $option;			
		}		
	...
	public static function categories($extension, $cat_id,$name="categories",$title="Select Category", $config = array('attributes'=>'class="inputbox"','filter.published' => array(0,1)))
	{
			...
			$query->from('#__tagok_categories AS a');
