...
defined('_JEXEC') or die('Restricted access');
  ...
  JToolBarHelper::title( JText::_( 'Kepviselojeloltek' ), 'generic.png' );
  JToolBarHelper::addNew();
  JToolBarHelper::editList();
  JToolBarHelper::publishList();
  JToolBarHelper::unpublishList();  
  JToolBarHelper::deleteList();  
  JToolBarHelper::preferences('com_kepviselojeloltek', '550');  
?>

<form action="index.php?option=com_kepviselojeloltek&amp;view=kepviselojeloltek" method="post" name="adminForm" id="adminForm">
	<table>
		<tr>
	...
	<table class="adminlist table table-striped">
		<thead>
			<tr>
				...
				<th class="title">
					<?php echo JHTML::_('grid.sort', 'User_id', 'a.user_id', $this->lists['order_Dir'], $this->lists['order'] ); ?>
				</th>				
				<th class="title">
					<?php echo JHTML::_('grid.sort', 'Temakor_id', 'a.temakor_id', $this->lists['order_Dir'], $this->lists['order'] ); ?>
				</th>				
			</tr>
		</thead>
		<tfoot>
...
<?php
  ...
  for ($i=0, $n=count( $this->items ); $i < $n; $i++):
    ...
    if (JRequest::getVar('function', null)) {
    	$onclick= "onclick=\"window.parent.jSelectKepviselojeloltek_id('".$row->id."', '".$this->escape($row->id)."', '','id')\" ";
    }  	
    
 	$link = JRoute::_( 'index.php?option=com_kepviselojeloltek&view=kepviselojeloltek&task=edit&cid[]='. $row->id );
 	$row->id = $row->id; 	
...
<input type="hidden" name="task" value="kepviselojeloltek" />
