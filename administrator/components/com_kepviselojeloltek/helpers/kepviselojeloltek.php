...
class KepviselojeloltekHelper
{
	
	/*
	 * Submenu for Joomla 1.6
	 */
	public static function addSubmenu($vName = 'kepviselojeloltek')
	{
        
		JSubMenuHelper::addEntry(
			JText::_('Kepviselojeloltek'),
			'index.php?option=com_kepviselojeloltek&view=kepviselojeloltek',
			($vName == 'kepviselojeloltek')
		);
	...
	public static function getExtensions() 
	{
		...
		$xml = simplexml_load_file(JPATH_ADMINISTRATOR.'/components/com_kepviselojeloltek/elements/extensions.xml', 'JXMLElement');		        
 ...
 * @since		1.5
 */
abstract class JHtmlKepviselojeloltek
{
	/**
	...
	{
		$extensions = KepviselojeloltekHelper::getExtensions();
		$options = array();
		...
		foreach ($extensions as $extension) {   
			...
			$option->value = 'com_kepviselojeloltek.'.$extension->name;
			$options[] = $option;			
		}		
	...
	public static function categories($extension, $cat_id,$name="categories",$title="Select Category", $config = array('attributes'=>'class="inputbox"','filter.published' => array(0,1)))
	{
			...
			$query->from('#__kepviselojeloltek_categories AS a');
