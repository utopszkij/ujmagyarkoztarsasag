...
class KepviselokHelper
{
	
	/*
	 * Submenu for Joomla 1.6
	 */
	public static function addSubmenu($vName = 'kepviselok')
	{
        
		JSubMenuHelper::addEntry(
			JText::_('Kepviselok'),
			'index.php?option=com_kepviselok&view=kepviselok',
			($vName == 'kepviselok')
		);
	...
	public static function getExtensions() 
	{
		...
		$xml = simplexml_load_file(JPATH_ADMINISTRATOR.'/components/com_kepviselok/elements/extensions.xml', 'JXMLElement');		        
 ...
 * @since		1.5
 */
abstract class JHtmlKepviselok
{
	/**
	...
	{
		$extensions = KepviselokHelper::getExtensions();
		$options = array();
		...
		foreach ($extensions as $extension) {   
			...
			$option->value = 'com_kepviselok.'.$extension->name;
			$options[] = $option;			
		}		
	...
	public static function categories($extension, $cat_id,$name="categories",$title="Select Category", $config = array('attributes'=>'class="inputbox"','filter.published' => array(0,1)))
	{
			...
			$query->from('#__kepviselok_categories AS a');
