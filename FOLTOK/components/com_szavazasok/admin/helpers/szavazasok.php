...
class SzavazasokHelper
{
	
	/*
	 * Submenu for Joomla 1.6
	 */
	public static function addSubmenu($vName = 'szavazasok')
	{
        
		JSubMenuHelper::addEntry(
			JText::_('Szavazasok'),
			'index.php?option=com_szavazasok&view=szavazasok',
			($vName == 'szavazasok')
		);
	...
	public static function getExtensions() 
	{
		...
		$xml = simplexml_load_file(JPATH_ADMINISTRATOR.'/components/com_szavazasok/elements/extensions.xml', 'JXMLElement');		        
 ...
 * @since		1.5
 */
abstract class JHtmlSzavazasok
{
	/**
	...
	{
		$extensions = SzavazasokHelper::getExtensions();
		$options = array();
		...
		foreach ($extensions as $extension) {   
			...
			$option->value = 'com_szavazasok.'.$extension->name;
			$options[] = $option;			
		}		
	...
	public static function categories($extension, $cat_id,$name="categories",$title="Select Category", $config = array('attributes'=>'class="inputbox"','filter.published' => array(0,1)))
	{
			...
			$query->from('#__szavazasok_categories AS a');
