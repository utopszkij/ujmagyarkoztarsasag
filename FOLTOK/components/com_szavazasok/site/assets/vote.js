...
/*
Müködés: 
  mouseMove grafikusan mozgatja a sort a meglévők fölött de még nem változtatja a select értékét
  mouseUp   átirja a table tartalmát, újra sorszámoz, biztositva, hogy a most elegenedett fölfött kisebb, 
  alatta nagyobb számok legyenek, ami eddig egyforma volt az egyforma is marad.


Terv 
  MouseUp -> selectedTr ez az a pozició ahol elengedte az egér gombot
             ennek a pos értéke legyen azonos a tr grafikus poziciójával
             halad felfelé a sorokoné 
...
<tr class="tied" onclick="select_row(this, event.shiftKey||event.ctrlKey);">
<td class="choice">Allow to submit weights for each voter, eg by integer number after voter's email (write-in)</td>
<td>
<select onchange="sort_rows()" name="C45" size="1">
...
// "a" tömb "i"-edit elemét átmozgatja "j" pozicióba
...
// be   num_choices darabszám
//      prefsection
//      selected = array 
// read_rows -> rank = array() poziciokat tartalmazó tömb
...
function do_make_tie() {
    ...
    if (num_selected < 2) {
        alert("Nincs elég sor kijelölve. "+
              "Shift-click  vagy ctrl-click -el lehet több sort ijelölni.");
        return;
    }
    for (var i = 0; i < num_choices; i++) {
        if (selected[i])
