...
defined('_JEXEC') or die('Restricted access');
  ...
  JToolBarHelper::title( JText::_( 'Temakorok' ), 'generic.png' );
  JToolBarHelper::addNew();
  JToolBarHelper::editList();
  JToolBarHelper::publishList();
  JToolBarHelper::unpublishList();  
  JToolBarHelper::deleteList();  
  JToolBarHelper::preferences('com_temakorok', '550');  
?>

<form action="index.php?option=com_temakorok&amp;view=temakorok" method="post" name="adminForm" id="adminForm">
	<table>
		<tr>
...
<?php
  ...
  for ($i=0, $n=count( $this->items ); $i < $n; $i++):
    ...
    if (JRequest::getVar('function', null)) {
    	$onclick= "onclick=\"window.parent.jSelectTemakorok_id('".$row->id."', '".$this->escape($row->megnevezes)."', '','id')\" ";
    }  	
    
 	$link = JRoute::_( 'index.php?option=com_temakorok&view=temakorok&task=edit&cid[]='. $row->id );
 	$row->id = $row->id; 	
...
<input type="hidden" name="task" value="temakorok" />
