...
class AlternativakHelper
{
	
	/*
	 * Submenu for Joomla 1.6
	 */
	public static function addSubmenu($vName = 'alternativak')
	{
        
		JSubMenuHelper::addEntry(
			JText::_('Alternativak'),
			'index.php?option=com_alternativak&view=alternativak',
			($vName == 'alternativak')
		);
	...
	public static function getExtensions() 
	{
		...
		$xml = simplexml_load_file(JPATH_ADMINISTRATOR.'/components/com_alternativak/elements/extensions.xml', 'JXMLElement');		        
 ...
 * @since		1.5
 */
abstract class JHtmlAlternativak
{
	/**
	...
	public static function categories($extension, $cat_id,$name="categories",$title="Select Category", $config = array('attributes'=>'class="inputbox"','filter.published' => array(0,1)))
	{
			...
			$query->from('#__alternativak_categories AS a');
