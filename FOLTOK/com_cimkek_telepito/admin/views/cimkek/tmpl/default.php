...
defined('_JEXEC') or die('Restricted access');
  ...
  JToolBarHelper::preferences('com_cimkek', '550');  
?>

<form action="index.php?option=com_cimkek&amp;view=cimkek" method="post" name="adminForm" id="adminForm">
	<table>
		<tr>
	...
	<table class="adminlist table table-striped">
		<thead>
			<tr>
				...
				<th class="title">
					<?php echo JHTML::_('grid.sort', 'Cimke', 'a.cimke', $this->lists['order_Dir'], $this->lists['order'] ); ?>
				</th>								<th class="title">
...
<?php
  ...
  for ($i=0, $n=count( $this->items ); $i < $n; $i++):
    ...
    if (JRequest::getVar('function', null)) {
    	$onclick= "onclick=\"window.parent.jSelectCimkek_id('".$row->id."', '".$this->escape($row->cimke)."', '','id')\" ";
    }  	
    
 	$link = JRoute::_( 'index.php?option=com_cimkek&view=cimkek&task=edit&cid[]='. $row->id );
 	$row->id = $row->id; 	
  ...
  ?>
	<tr class="<?php echo "row$k"; ?>">
        ...
        <td>
							...
							<a <?php echo $onclick; ?>href="<?php echo $link; ?>"><?php echo $row->cimke; ?></a>
...
<input type="hidden" name="option" value="com_cimkek" />
