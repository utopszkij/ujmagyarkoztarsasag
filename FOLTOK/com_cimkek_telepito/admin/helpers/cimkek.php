...
class CimkekHelper
{
	 ...
	 */
	public static function addSubmenu($vName = 'cimkek')
	{
        
		JSubMenuHelper::addEntry(
			JText::_('Cimkek'),
			'index.php?option=com_cimkek&view=cimkek',
			($vName == 'cimkek')
		);
	...
	public static function getExtensions() 
	{
		...
		$xml = simplexml_load_file(JPATH_ADMINISTRATOR.'/components/com_cimkek/elements/extensions.xml', 'JXMLElement');		        
...
abstract class JHtmlCimkek
{
	...
	{
		$extensions = CimkekHelper::getExtensions();
		$options = array();
		...
		foreach ($extensions as $extension) {   
			...
			$option->value = 'com_cimkek.'.$extension->name;
			$options[] = $option;			
		}		
	...
	public static function categories($extension, $cat_id,$name="categories",$title="Select Category", $config = array('attributes'=>'class="inputbox"','filter.published' => array(0,1)))
	{
			...
			$query->from('#__cimkek_categories AS a');
