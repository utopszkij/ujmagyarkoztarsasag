...
* @package        Joomla site
* @subpackage amcomponent masodik controller
 ...
 */
class MasodikController extends JControllerLegacy {
    protected $viewName = '';
    ...
    foreach ($item0 as $fn => $fv) {
      ...
      if ($field) {
        if ($field->type == 'editor')
           $item[$fn] = JRequest::getVar($fn,'','POST','STRING',JREQUEST_ALLOWHTML);
        else
           $item[$fn] = JRequest::getVar($fn,'');
      }   
    }
    // checkboxok kezelése
