...
class SzavazasokModelPublikalandok extends JModelList
{
	...
	protected function getListQuery()	{
		...
		if (Jrequest::getVar('temakor') > 0)
		    $query .= ' where sz.temakor_id="'.JRequest::getVar('temakor',0).'" and sz.elbiralas_alatt=1 and sz.elutasitva = "" ';
		else
			$query .= ' where sz.temakor_id > 0 and sz.elbiralas_alatt=1 and sz.elutasitva = "" ';
		if ($filterStr=='')
			$query .= ' and sz.lezart < '.$lezartLimit;
		else
			$query .= ' and  (sz.megnevezes like "%'.$filterStr.'%" or c.cimke = "'.$filterStr.'") and sz.lezart < '.$lezartLimit;
		if ((JRequest::getVar('order')=='') | (JRequest::getVar('order') < 1)) JRequest::setVar('order',2);
